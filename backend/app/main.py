...
def get_recent_builds(limit: int = Query(20, ge=1, le=100), db: Session = Depends(get_db)):
    """
    Returns the most recent builds, ordered by build number and start time descending.
    """
    builds = db.query(Build).order_by(Build.started_at.desc(), Build.build_number.desc()).limit(limit).all()
    return [b.to_dict() for b in builds]

@app.get("/api/stats/failure-rate")
def get_failure_rate(days: int = Query(7, ge=1, le=90), db: Session = Depends(get_db)):
    """
...
def get_top_failing_jobs(db: Session = Depends(get_db)):
    """
